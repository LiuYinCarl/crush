...
- **消息搜索跳转**：聊天区焦点下（`Tab` 切换焦点）按 `/`
  ...
  搜索框内按 `Tab` 循环切换角色过滤：全部 → 仅 user → 仅 assistant。
- **用户消息跳转**：聊天区焦点下按 `K` / `J`
...
上游发布新 tag（例如 `v0.93.0`）后（无需创建发布分支，tag 直接挂在
cherry-pick 出的游离 commit 上）：

```bash
git fetch origin --tags

# 在新 tag 上脱离 HEAD，把补丁 cherry-pick 上来（如有冲突，解决后继续）
git checkout --detach v0.93.0
