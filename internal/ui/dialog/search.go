...
func NewSearch(com *common.Common, entries []MessageSearchEntry) *Search {
	s := &Search{com: com, entries: entries}
	...
	s.keyMap.CycleRole = key.NewBinding(
		key.WithKeys("tab"),
		key.WithHelp("tab", "filter role"),
	)
...
// rebuildList recreates the filterable list from the entries matching the
// current role filter, preserving the current query text.
func (s *Search) rebuildList() {
	var items []list.FilterableItem
	for _, entry := range s.entries {
		if s.roleFilter != "" && entry.Role != s.roleFilter {
			continue
		}
...
func (s *Search) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, s.keyMap.Close):
			return ActionClose{}
		case key.Matches(msg, s.keyMap.CycleRole):
			// Cycle: all -> user -> assistant -> all.
			switch s.roleFilter {
			case "":
				s.roleFilter = "user"
			case "user":
				s.roleFilter = "assistant"
			default:
				s.roleFilter = ""
			}
			s.rebuildList()
